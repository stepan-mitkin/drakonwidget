function createDrakonWidget() {
    ...
    function drakon_canvas() {
        ...
        function setFillStroke(config, node, ctx) {
            var theme, lineWidth, style, segments, shadowColor, _var2, _var3, _var4, _var5;
            var __state = '2';
            while (true) {
                switch (__state) {
                    case '2':
                    ...
                    case '35':
                        ...
                        if (shadowColor) {
                            ...
                            _var3 = getThemeValue(config, node, 'shadowBlur');
                            ctx.shadowBlur = _var3 * config.zoom;
                            _var4 = getThemeValue(config, node, 'shadowOffsetX');
                            ctx.shadowOffsetX = _var4 * config.zoom;
                            _var5 = getThemeValue(config, node, 'shadowOffsetY');
                            ctx.shadowOffsetY = _var5 * config.zoom;
                            __state = '17';
                        } else {
                            ctx.shadowColor = 'rgba(0, 0, 0, 0)';
                            ctx.shadowBlur = 0;
        ...
        function findFreePos(widget, evt) {
            var mousePos, leftPos, visuals, config;
            visuals = widget.visuals;
            config = visuals.config;
            ...
            while (true) {
                switch (__state) {
                    ...
                    case '21':
                        if (_var6 < _var5.length) {
                            element = _var5[_var6];
                            calculateBoxFromFree(element, box);
                            _var6++;
                            __state = '21';
                        } else {
                            __state = '34';
        ...
        function calculateBoxFromFree(element, box) {
            var left, top, right, bottom, _var2, _var3, _var4, _var5;
            var __state = '2';
            while (true) {
                switch (__state) {
                    ...
                    case '2':
                        ...
                        top = element.box.top;
                        right = left + element.box.width;
                        bottom = top + element.box.height;
                        _var2 = isNaN(left);
                        if (_var2) {
                            __state = '_item3';
                        } else {
        ...
        function sanitizeScroll(widget, x, y) {
            var visuals, box, x2, y2, zoom, x3, y3;
            visuals = widget.visuals;
            box = visuals.box;
            zoom = widget.zoomFactor;
            x2 = Math.min(box.right - widget.width / zoom, x);
            x3 = Math.max(box.left, x2);
            y2 = Math.min(box.bottom - widget.height / zoom, y);
        ...
        function paintCore(widget, factor, zoom, translateX, translateY, width, height) {
            var padding, element, visuals, config, ctx, _var3, _var2, _var4, id, edge, _var9, _var8, _var10, node, _var6, _var5, _var7, type, _var12, _var11, _var13, _var19, _var20, socket, _var14, _var15, _var17, _var16, _var18, _var21;
            var __state = '2';
            while (true) {
                switch (__state) {
                    case '2':
        ...
        function flowFreeIcon(widget, visuals, element) {
            var elementActions;
            elementActions = widget.freeIcons[element.type];
            elementActions.flow(visuals, element);
            return;
        }
            ...
            while (true) {
                switch (__state) {
                    case '2':
                        ...
                        color = visuals.config.theme.lines;
                        radius = 10;
                        top = element.y + element.hiY;
                        bottom = element.y + element.loY;
                        ctop = top + radius;
                        cbottom = bottom - radius;
                        tx = element.x + element.hiX;
                        bx = element.x + element.loX;
                        ctx.strokeStyle = color;
                        ctx.lineWidth = thickness;
                        dash = [
                            15,
                    ...
                    case '19':
                        ...
                        ctx.lineTo(element.x, ctop);
                        ctx.stroke();
                        ctx.beginPath();
                        ctx.moveTo(element.x, element.y);
                        ctx.lineTo(element.x, cbottom);
                        ctx.stroke();
                        if (element.flag1) {
                            __state = '23';
                        } else {
        ...
        function pasteFree(widget, clipboard) {
            var edits, firstId, create, payload, pos, _var2;
            payload = clipboard.content;
            edits = [];
